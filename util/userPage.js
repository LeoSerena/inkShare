...
class Row extends React.Component {
    ...
    render(){
        if(this.props.isheader){return <tr>
            {this.props.list.map(e =><th key={e} onClick={() => this.props.sortFunc(e)}>{e}</th>)}<th><AddBookForm/></th>
        </tr>} 
        else{return (<tr>
            {this.props.list.map(elem => <td key={elem} onClick={() => this.props.toggle(this.props.ID)}>{elem}</td>)}
